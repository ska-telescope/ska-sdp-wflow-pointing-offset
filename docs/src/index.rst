...
fitting the primary beam (modelled by a 2D Gaussian) to the cross-correlation visibility amplitudes of each antenna.
Note that the final pipeline would provide the option to fit the primary beams to the complex gains of each antenna (derived possibly from
the gain solver in `RASCIL`_). These set of functions used in this pipeline are based on those used by the `SARAO`_ team for computing pointing
offsets for the MeerKAT array.
